...
const AddSheet = () => {
  const { userData, equipment_working_hrs, sheet_code, equipment } = usePage().props

  console.log(equipment_working_hrs)
  ...
  const [summaryStatus, setSummaryStatus] = useState(0)
  ...
  useEffect(() => {
    if (filterType == 'today') {
      axios.get(route('eq.sheet.today', equipment.eq_id)).then(res => setAPIData(res.data.data))
    }

    if (filterType == 'month') {
      axios.get(route('eq.sheet.month', [equipment.eq_id, month])).then(res => setAPIData(res.data.data))
    }

    if (filterType == 'day') {
      axios.get(route('eq.sheet.day', [equipment.eq_id, day])).then(res => setAPIData(res.data.data))
    }

    if (filterType == 'both') {
      axios.get(route('eq.sheet.both', [equipment.eq_id, month, day])).then(res => setAPIData(res.data.data))
    }
  }, [filterType, month, day]);


  const createSheet = () => {
    Inertia.post(route('equipment.sheets.add', equipment.eq_id), {
      filterType: filterType,
      month: month,
      day: day,
      includeSummary: summaryStatus
    })
  }
  ...
  return (
    <Layout>
      ...
      <div className="layout-header">
        <h1>{translate('createSheet', userData.language)} {translate('forEquipment', userData.language)} -  <b>{equipment.eq_name}</b></h1>
      </div>

      {equipment_working_hrs.length > 0 ? (
          ...
          <div className="display-data">
            ...
            <div className="form-container">
              <div className="form-group">
                ...
                <Input value={equipment.eq_name + ' #' + equipment.eq_id} disabled={true} />
              </div>
              ...
              <div className="form-group" style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <label className="switch">
                  <input type="checkbox" onChange={ e => setSummaryStatus(e.target.checked) } />
            ...
            <div className="tabs-content">
              ...
              {tabs.today && (
                <div className="tab-content">
                  <h1>{translate('todaySheet', userData.language)} {translate('forEquipment', userData.language)} - {equipment.eq_name}</h1>
                  <p>{translate('generatedEqSheetParagraphToday', userData.language)}</p>
                </div>
              )}

              {tabs.month && (
                <div className="tab-content">
                  <h1>{translate('monthSheet', userData.language)} {translate('forEquipment', userData.language)} - {equipment.eq_name}</h1>
                  <p>{translate('generatedEqSheetParagraphMonth', userData.language)}</p>
                  <div className="form-group">
              ...
              {tabs.day && (
                <div className="tab-content">
                  <h1>{translate('daySheet', userData.language)} {translate('forEquipment', userData.language)} - {equipment.eq_name}</h1>
                  <p>{translate('generatedEqSheetParagraphDay', userData.language)}</p>
                  <div className="form-group">
              ...
              {tabs.both && (
                <div className="tab-content">
                  <h1>{translate('bothSheet', userData.language)} {translate('forEquipment', userData.language)} - {equipment.eq_name}</h1>
                  <p>{translate('generatedEqSheetParagraphBoth', userData.language)}</p>
                  <div className="form-group">
                    <Label value={translate('month', userData.language)} />
                    <select name="" id="" className='form-select' onChange={(e) => setMonth(e.target.value)}>
                      <option value=''>Choose Month to Filter</option>
              ...
              {openData && (
                <div className="table-container" style={{ width: '100%', height: '500px' }}>
                  <DataGrid columns={EquipmentsWorkingHoursColumns} rows={apiData} getRowId={ row => row.eq_hr_id } />
                </div>
              )}
